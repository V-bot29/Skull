...
class Game:
    def __init__(self, player_amount = 0, game_bots = []):
        ...
        for i in range(1, self.player_amount + 1):
            self.initial_gamestate["players"].append(Player(i))
        for j in range(self.player_amount+1, self.player_amount + self.bot_amount + 1):
            new_player = Player(j, is_bot = True)
            self.initial_gamestate["players"].append(new_player)

            # Bot takes control of player
            self.game_bots[j - self.player_amount - 1].player = new_player
        
        self.initial_gamestate["player_to_act"] = self.initial_gamestate["players"][0]  # Who goes first

        print("Game initialized")
        
    def start_new_game(self):
        # Reset gamestate
        self.gamestate = copy.deepcopy(self.initial_gamestate)
    ...
    def flip_loop(self):
        ...
        for card in self.gamestate["player_to_act"].stack:
            if card.suit == 'flower':
                ...
                print(f'{self.gamestate["player_to_act"].name} flipped a flower from {self.gamestate["player_to_act"].name}')
            elif card.suit == 'skull':
                self.gamestate["skulls_flipped"] += 1
                print(f'{self.gamestate["player_to_act"].name} flipped a skull from {self.gamestate["player_to_act"].name}')
                return False
                    ...
                    if (player.name == player_id) and (player != self.gamestate["player_to_act"]):
                        ...
                        for card in player.stack:
                            if not card.flipped and (cards_this_flip < int(action_value)):
                                ...
                                if card.suit == 'flower':
                                    self.gamestate["flowers_flipped"] += 1
                                    print(f'{self.gamestate["player_to_act"].name} flipped a flower from {player.name}')
                                elif card.suit == 'skull':
                                    self.gamestate["skulls_flipped"] += 1
                                    print(f'{self.gamestate["player_to_act"].name} flipped a skull from {player.name}')
                                    return False
    ...
    def turn_loop(self):
        ...
        while not valid_action: 
            ...
            if valid_action == False:
                print("Invalid action from",player.name, action_type, action_value)
    ...
    def process_action(self, player, action):
        ...
        if action == "pass" or (action == "pass choice" or action == "pass forced"):
    ...
    def bot_action(self):  
        ...
        for bot in self.game_bots:
            if bot.player.name == self.gamestate["player_to_act"].name:
                move = bot.get_move(self.gamestate)
    ...
    def get_moves(self, gamestate = None):
        ...
        if gamestate["flipping_phase"] == True:
            ...
            if len(player.hand) == 0 and gamestate["carding_phase"]:
                ...
                for value in bet_values:
                    moves.append(("bet", value))
                moves.append(("pass", "choice"))

            elif gamestate["betting_phase"] and (gamestate['highest_bet'] == gamestate["cards_played"]):
                moves.append(("pass", "forced"))
                # TODO sould not be activated
...
class SimGame(Game):
    def __init__(self, input_gamestate):
        self.original_gamestate = copy.deepcopy(input_gamestate)
        self.gamestate = copy.deepcopy(input_gamestate)
        self.gamestate_history = [copy.deepcopy(self.original_gamestate)]
        self.player_to_act = self.gamestate["player_to_act"]
        self.next_player_name = self.gamestate["player_to_act"].name

    def do_move(self, move):
        #print(f'Player {self.gamestate["player_to_act"].name} Action: {move}')
        action_type, action_value = move
        # Change Gamestate
        ...
        if not self.gamestate["game_over"]:
            ...
            if not self.gamestate["round_over"]:
                ...
                if self.gamestate["flipping_phase"] == True:             
    ...
    def undo_move(self):
        ...
        self.gamestate = copy.deepcopy(self.gamestate_history[-1])
        return self.gamestate

    def reset(self):
        self.gamestate = copy.deepcopy(self.original_gamestate)
        self.gamestate_history = [copy.deepcopy(self.gamestate)]
        return self.gamestate
    
    def is_terminal(self):

        if self.get_moves(self.gamestate) == []:
            return True
